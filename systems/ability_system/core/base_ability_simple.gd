...
func _start_cooldown(holder) -> void:
	if base_cooldown <= 0:
		return
	is_on_cooldown = true
...
func get_modified_value(base_value: float, stat_name: String, holder) -> float:
	return base_value
