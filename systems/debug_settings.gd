...
## 
## REMOVAL: To remove debug panel, delete this file and all references to DebugSettings.instance
## See: guides/DEBUG_PANEL_REMOVAL_GUIDE.md

static var instance: DebugSettings

const SAVE_PATH = "user://debug_settings.cfg"
const DEBUG_PANEL_ENABLED = true  # Set to false to disable all debug functionality
...
## Helper to check if debug is active and a setting is disabled
static func is_disabled(setting: String) -> bool:
	if not instance or not DEBUG_PANEL_ENABLED:
		return false  # Debug off = everything enabled
	return not instance.get(setting)
